...
class Cityscapes(data.Dataset):
    def __init__(self, root_dir, set='train', max_iters=None, img_size=640):
        ...
        # Build SF listing (for random sampling)
        self.sf_ids = [f for f in os.listdir(self.sf_image_dir) if f.endswith('.png')]

        if max_iters is not None:
            self.sf_ids = self.sf_ids * int(np.ceil(float(max_iters) / len(self.sf_ids)))
        random.shuffle(self.img_ids) # IMPORTANT

        self.files = []
        for img_id, sf_id in zip(self.img_ids, self.sf_ids):
            self.files.append({
                'cw_img': osp.join(self.cw_image_dir, img_id),
                'sf_img': osp.join(self.sf_image_dir, sf_id),
                'cw_label': osp.join(self.cw_label_dir, img_id.replace('.png', '.txt')),
                'sf_label': osp.join(self.sf_label_dir, sf_id.replace('.png', '.txt')),
                'name': img_id
            })
